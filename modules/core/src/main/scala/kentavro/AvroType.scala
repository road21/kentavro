...
import kentavro.AvroType.Tag.TaggedJValue
...
trait AvroType[T]:
  ...
  def JTypeDescription: String

  def schema: Schema

  def serialize(data: T): Array[Byte] =
    AvroType.serializeUnsafe(serializeToJType(data), schema)

  def deserialize(bytes: Array[Byte]): Either[String, T] =
    AvroType.deserializeUnsafe(bytes, schema).flatMap {
      x =>
        isJType(x) match
          case Some(j) => deserializeFromJType(j)
          case _       => Left(
              s"Expected $JTypeDescription, got ${x.toString()}: ${x.getClass()}"
            )
    }
...
object AvroType:
  ...
  trait NamedSchema[Name <: Singleton & String, T](using ValueOf[Name]) extends AvroType[Name ~ T]:
    final val name: Name = valueOf

  trait Primitive[_JType <: AnyRef, SType](val tag: Tag)(using tag.JType =:= _JType, tag.SType =:= SType)
    extends AvroType[SType]:
    override type JType = _JType

    override val JTypeDescription: String = tag.toString
    ...
    override def deserializeFromJType(
      ...
      val (errs, succ) = fields.partitionMap { f =>
        ...
        f.schema.isJType(v) match
          ...
          case None    => Left(s"Expected ${f.schema.JTypeDescription}, got: $v")
      }
      Either.cond(
        errs.isEmpty,
    ...
    override def deserializeFromJType(obj: java.util.Collection[?]): Either[String, Vector[T]] =
      ...
      val (errs, succs) = arr.partitionMap { x =>
        element.isJType(x) match
          ...
          case None    => Left(s"Expected ${element.JTypeDescription}, got: $x")
      }
      ...
      val (errs, succs) = m.partitionMap {
        case (key, v) =>
          value.isJType(v) match
            ...
            case None    => Left(s"Expected ${value.JTypeDescription}, got: $key")
      }
  ...
  case class UnionSchema[T](
      override val schema: Schema,
      variants: SeqMap[Tag, AvroType[?]]
  ) extends AvroType[T]:
    override type JType = AnyRef

    override val JTypeDescription: String = variants.map((k, _) => k).mkString("|")
    ...
    override def serializeToJType(data: T): AnyRef =
      Tag.fromScala(data).flatMap { tv =>
        variants.get(tv.t).map { case (s: AvroType[typ]) => s.serializeToJType(data.asInstanceOf[typ]) }
