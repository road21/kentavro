...
object NamedCompanion:
  class Record[Name <: String & Singleton, Base <: AnyNamedTuple](override val name: Name) extends NamedCompanion[Name, Base]:
    type Type = Name ~ Base

    def apply(d: Base): (Name ~ Base) = Named.make(d)(using ValueOf(name))

  class Enum[Name <: String & Singleton, V <: Tuple](override val name: Name) extends NamedCompanion[Name, V], Selectable:
    type Type = Name ~ V
